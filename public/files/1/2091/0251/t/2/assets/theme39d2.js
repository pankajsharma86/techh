...
theme.Drawers = (function() {
  ...
  Drawer.prototype = $.extend({}, Drawer.prototype, {
    ...
    bindEvents: function() {
      ...
      this.$nodes.page.on('touchmove' + this.config.namespace, function () {
        return false;
      });
...
theme.collapsibles = (function() {
  ...
  function setTransitionHeight($module, height, isOpen, isAutoHeight) {
    ...
    if (!isOpen && isAutoHeight) {
      var o = $module;
      window.setTimeout(function() { o.css('height','auto'); }, 500);
    }
  }
...
theme.headerNav = (function() {
  ...
  function fitNav() {
    ...
    var navWidth = $nav.parent().outerWidth() - 20;
    var navItemWidth = 0;
    ...
    if (navItemWidth > navWidth) {
      $nav.addClass('site-nav--compress');
    } else {
      $nav.removeClass('site-nav--compress');
    }
  }
...
theme.currencySwitcher = (function() {
  ...
  function init() {
    ...
    // Keep mobile footer picker in sync
    if ($('.footer__currency select').length) {
      $('.footer__currency select').on('change', function() {
        $('#CurrencyPicker').val($(this).val()).trigger('change');
      });
    }
...
theme.videoModal = function() {
  ...
  var selectors = {
    triggers: 'a[href*="youtube.com/watch"]'
  };
...
theme.Product = (function() {
  ...
  function onVideoPlayerReady(evt) {
    ...
    if ($player.closest('.slick-current').length || $player.closest('.product-image-main')) {
  ...
  function Product(container) {
    ...
    this.settings = {
      ...
      modalInit: false,
      lazyLoadModalImages: $container.data('modal-images') || false,
      modalImagesLoaded: false,
    ...
    this.selectors = {
      ...
      singleOptionSelector: '.product__option-radio-' + sectionId,
  ...
  Product.prototype = $.extend({}, Product.prototype, {
    ...
    setImageSizes: function() {
      ...
      this.settings.imageSize = theme.Images.imageSize(this.$firstProductImage.data('image-src'));
    ...
    updatePrice: function(evt) {
      ...
      if (variant) {
        ...
        if (variant.compare_at_price > variant.price) {
          $(this.selectors.comparePrice).text(theme.Currency.formatMoney(variant.compare_at_price, theme.settings.moneyFormat));
    ...
    playVimeoVideo: function(id) {
      ...
      this.settings.playingVideo = id;
    },

    stopVimeoVideo: function(id) {
      if (!id && this.settings.playingVideo && vimeoPlayers[this.settings.playingVideo]) {
        if (typeof vimeoPlayers[this.settings.playingVideo].pause === 'function') {
          vimeoPlayers[this.settings.playingVideo].pause();
        }
        return;
      }
    ...
    requestToPlayYoutubeVideo: function(id) {
      if (!theme.config.youTubeReady) {
        return;
      }

      if (this.settings.playingVideo) {
        this.stopYoutubeVideo(this.settings.playingVideo);
      }
    ...
    stopYoutubeVideo: function(id) {
      ...
      if (!id && this.settings.playingVideo && youtubePlayers[this.settings.playingVideo]) {
        if (typeof youtubePlayers[this.settings.playingVideo].pauseVideo === 'function') {
          youtubePlayers[this.settings.playingVideo].pauseVideo();
        }
        return;
      }
    ...
    playMp4Video: function(id) {
      ...
      if (this.settings.playingVideo) {
        this.stopMp4Video(this.settings.playingVideo);
      }
    ...
    stopMp4Video: function(id) {
      if (!id && this.settings.playingVideo) {
        var oldPlayer = $('#' + this.settings.playingVideo)[0];
    ...
    createImageCarousels: function() {
      ...
      // Init zoom on each lazy loaded image
      this.$mainSlider.on('lazyLoaded', function(evt, $slider, $image, imageSource) {
        if (this.settings.zoom) {
          this.initZoom($image);
        }
      }.bind(this));
    ...
    mainSlideInit: function(event, slick) {
      ...
      var $video = $currentSlide.find('.product__video');
    ...
    beforeSlideChange: function(event, slick, currentSlide, nextSlide) {
      ...
      var $video = $nextSlide.find('.product__video');
      var videoId;
      var videoType;

      if ($video.length) {
        videoType = $video.data('video-type');
        videoId = $video.attr('id');

        if (videoId && videoType === 'youtube') {
          if (videos[videoId].style === 'muted') {
            this.requestToPlayYoutubeVideo(videoId);
          }
        }

        if (videoId && videoType === 'mp4') {
          this.playMp4Video(videoId);
        }

        if (videoId && videoType === 'vimeo') {
          this.autoplayVimeoVideo(videoId);
        }
      }
    ...
    loadModalImages: function() {
      if (!this.settings.modalImagesLoaded) {
        ...
        this.$modal.find('img[data-modal-lazy]').each(function(i, img) {
          ...
          if (loadCount == imageCount) {
            this.settings.modalImagesLoaded = true;
          }
        }.bind(this));
      }
    },
...
theme.Collection = (function() {
  var classes = {
    ...
    productTemplateDiv: '.product-section',
...
theme.VideoSection = (function() {
  ...
  function videoSection(container) {
    ...
    var mp4Video = this.mp4Video = 'Mp4Video-' + this.sectionId;
  ...
  function onVideoPlayerReady(evt) {
    ...
    checkVisibility(playerId);
    $(window).on('scroll.' + playerId, {id: playerId}, $.throttle(100, checkVisibility));

    // Add better background sizing
  }
  ...
  function checkVisibility(playerId) {
    var id;
    if (typeof playerId === 'string') {
      id = playerId;
    } else {
      // Actually comes in as part of the scroll event data
      id = playerId.data.id;
    }

    var $videoElement = $('#' + id);

    if (theme.isElementVisible($videoElement) ) {
      if (typeof youtubePlayers[id].playVideo === 'function') {
        youtubePlayers[id].playVideo();
      }
    } else {
...
theme.Maps = (function() {
  ...
  Map.prototype = $.extend({}, Map.prototype, {
    createMap: function() {
      ...
      console.log($map);
      console.log($map.data('address-setting'));
      ...
      return geolocate($map)
        ...
        .fail(function() {
          var errorMessage = theme.strings.addressError;
...
theme.setGlobals = function() {
  theme.config.hasSessionStorage = theme.isSessionStorageSupported() || theme.inIframe();
